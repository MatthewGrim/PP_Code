...
def get_ffp_stability_condition(plot_results=True):
    ffp_factors = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0]
    deltas = list()
    alphas = list()
    for factor in ffp_factors:
        # Modify and plot input profiles
        dat = np.loadtxt(os.path.join('FFp_profiles', 'FFp_scan_-{}'.format(factor), 'input_profiles.dat'))
        r = dat[:, 0]
        R = dat[:, 1]
        j_phi = dat[:, 2]
        B_z = dat[:, 3]
        ...
        # Skip profile if the instability does not exist
        if np.max(q) < 2:
            continue
    ...
    plt.ylabel('$\Delta$')
    plt.xlabel('$\\alpha$')
    plt.title('Stability profile for cylindrical plasma with varying FFprime')
    plt.savefig('ffp_stability')
    plt.show()
