...
from plasma_physics.pysrc.theory.coulomb_collisions.relaxation_processes import MaxwellianRelaxationProcess
...
def generate_sim_results(number_densities, T):
    ...
    for j, name in enumerate(names):
        ...
        for i, n in enumerate(number_densities):
            ...
            # Set up beam velocities
            velocities = np.zeros((2 * N, 3))
            velocities[:N, :] = np.asarray([0.0, 0.0, beam_velocity])

            # Set up background velocities
            k_T = PhysicalConstants.boltzmann_constant * T
        ...
        # Save results
        np.savetxt("maxwellian_{}_{}_{}_half_times".format(name, N, dt_factor), t_halves[name])
        np.savetxt("maxwellian_{}_{}_{}_velocities".format(name, N, dt_factor), velocity_results[name])
        np.savetxt("maxwellian_{}_{}_{}_energies".format(name, N, dt_factor), energy_results[name])
...
if __name__ == '__main__':
    ...
    temperature = 1000.0 * UnitConversions.eV_to_K
