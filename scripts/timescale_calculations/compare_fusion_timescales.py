...
from plasma_physics.pysrc.theory.reactivities.fusion_reactivities import BoschHaleReactivityFit, FusionReaction, \
    DDReaction, DTReaction, pBReaction
...
def get_thermalisation_rate(n, T, relaxation_process, beam_velocity, reaction_name, force_calculation=False):
    ...
    if isinstance(n, np.ndarray):
        ...
        for i, number_density in enumerate(n):
            print(i)
            theoretical_maxwellian_frequency[i] = relaxation_process.maxwellian_collisional_frequency(number_density, T,
                                                                                                      beam_velocity)
            numerical_maxwellian_frequency[i] = relaxation_process.numerical_kinetic_loss_maxwellian_frequency(
                number_density, T, beam_velocity)
            monte_carlo_maxwellian_frequency[i] = relaxation_process.monte_carlo_kinetic_loss_maxwellian_frequency(
                number_density, T, beam_velocity)
    else:
        theoretical_maxwellian_frequency = relaxation_process.maxwellian_collisional_frequency(n, T, beam_velocity)
        numerical_maxwellian_frequency = relaxation_process.numerical_kinetic_loss_maxwellian_frequency(n, T,
                                                                                                        beam_velocity)
        monte_carlo_maxwellian_frequency = relaxation_process.monte_carlo_kinetic_loss_maxwellian_frequency(
            number_density, T, beam_velocity)
...
if __name__ == '__main__':
    ...
    t_stationary, t_maxwellian, t_numerical, t_monte = get_thermalisation_rate(n, T_K, relaxation_process,
                                                                               beam_velocity,
