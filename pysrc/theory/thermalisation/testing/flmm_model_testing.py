...
from pysrc.theory.thermalisation.flmm_model import FLMMmodel
...
def paper_comparison():
	...
	ax[0].loglog(temperatures, lamda_numerical)
	ax[0].loglog(lamda_data[:, 0],lamda_data[:, 1])

	ax[1].semilogy(final_ion_energy, temperatures)
	ax[1].semilogy(u_ion_data[:, 0], u_ion_data[:, 1])
